...
> [!IMPORTANT]
> ## 📂 Context-Aware Source Loading (avoid context-length errors)
>
> When the source content was generated by a previous task (e.g., deep-research output) and exists as a **file in the sandbox** (`/mnt/user-data/outputs/` or `/mnt/user-data/workspace/`), **always read it from the file** using `read_file` instead of relying on conversation history. Do NOT paste the full document content inline — reference the file path.
>
> **Why:** Large research documents (20K+ tokens) will exceed the model's context window if carried in conversation history. Reading from the file keeps the context clean and only loads what you need.
>
> **How to detect:** If the user says "基于这个/刚才的/上面的 生成PPT", check if an artifact file (`.md`) was presented in an earlier step. If yes, use `read_file <path>` to load it. If the user uploaded a file, use the virtual path from the attachment context.

> [!IMPORTANT]
> ## 📄 Template PPTX handling (uploaded custom templates)
>
> When the system context contains `template_pptx: /path/to/file.pptx`, the user has uploaded a raw PowerPoint file to use as a **visual style template**. The template's original content is IRRELEVANT — it provides the visual framework (colors, fonts, layouts, slide structure). All original content WILL BE REPLACED.
>
> **⚠️ CRITICAL — NEVER skip or fall back to free design when `template_pptx` is present.** Use the template unconditionally, regardless of content match.
>
> **How to handle:**
> 1. **Skip Step 3 (Template Option)** — `template_pptx` is a raw PPTX file, not a template directory
> 2. **DO NOT judge whether the template "matches" the topic** — it's a visual skeleton, content match is irrelevant
> 3. **Ignore `style` / `industry` / `template` parameters** — the template IS the visual style
> 4. Use the **[`template-fill`](workflows/template-fill-pptx.md)** workflow:
>    - `python3 ${SKILL_DIR}/scripts/template_fill_pptx.py analyze <template_pptx_path> -o slide_library.json`
>    - Use the slide library as the page roster
>    - Fill ALL slides with the user's topic, replacing all original text
>
> `template_pptx` takes precedence over `template_path`. If both are present, use `template_pptx`.

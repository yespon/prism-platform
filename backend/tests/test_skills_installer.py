...
class TestInstallSkillFromArchive:
    ...
    def _make_zip_archive(self, tmp_path: Path, skill_name: str = "zip-skill") -> Path:
    ...
    def test_success(self, tmp_path):
        ...
        assert result["relative_path"] == "test-skill"
        assert (skills_root / "custom" / "test-skill" / "SKILL.md").exists()

    def test_success_with_relative_parent_path(self, tmp_path):
        zip_path = self._make_skill_zip(tmp_path, skill_name="tenant-skill")
        skills_root = tmp_path / "skills"
        skills_root.mkdir()
        result = install_skill_from_archive(
            zip_path,
            skills_root=skills_root,
            target_relative_path="tenant-a",
        )
        assert result["success"] is True
        assert result["skill_name"] == "tenant-skill"
        assert result["relative_path"] == "tenant-a/tenant-skill"
        assert (skills_root / "custom" / "tenant-a" / "tenant-skill" / "SKILL.md").exists()
    ...
    def test_accepts_zip_extension(self, tmp_path):
        zip_path = self._make_zip_archive(tmp_path)
        skills_root = tmp_path / "skills"
        skills_root.mkdir()
        result = install_skill_from_archive(zip_path, skills_root=skills_root)
        assert result["success"] is True
        assert result["skill_name"] == "zip-skill"
        assert (skills_root / "custom" / "zip-skill" / "SKILL.md").exists()

    def test_invalid_extension(self, tmp_path):
        bad_path = tmp_path / "bad.rar"

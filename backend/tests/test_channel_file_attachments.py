...
class TestResolveAttachments:
    def test_resolves_existing_file(self, tmp_path):
        ...
        with patch("deerflow.config.paths.get_paths", return_value=mock_paths):
            result = _resolve_attachments("local", thread_id, ["/mnt/user-data/outputs/report.pdf"])
    ...
    def test_resolves_image_file(self, tmp_path):
        ...
        with patch("deerflow.config.paths.get_paths", return_value=mock_paths):
            result = _resolve_attachments("local", thread_id, ["/mnt/user-data/outputs/chart.png"])
    ...
    def test_skips_missing_file(self, tmp_path):
        ...
        with patch("deerflow.config.paths.get_paths", return_value=mock_paths):
            result = _resolve_attachments("local", "t1", ["/mnt/user-data/outputs/nonexistent.txt"])
    ...
    def test_skips_invalid_path(self):
        ...
        with patch("deerflow.config.paths.get_paths", return_value=mock_paths):
            result = _resolve_attachments("local", "t1", ["/invalid/path"])
    ...
    def test_rejects_uploads_path(self):
        ...
        with patch("deerflow.config.paths.get_paths", return_value=mock_paths):
            result = _resolve_attachments("local", "t1", ["/mnt/user-data/uploads/secret.pdf"])
    ...
    def test_rejects_workspace_path(self):
        ...
        with patch("deerflow.config.paths.get_paths", return_value=mock_paths):
            result = _resolve_attachments("local", "t1", ["/mnt/user-data/workspace/config.py"])
    ...
    def test_rejects_path_traversal_escape(self, tmp_path):
        ...
        with patch("deerflow.config.paths.get_paths", return_value=mock_paths):
            result = _resolve_attachments("local", thread_id, ["/mnt/user-data/outputs/../uploads/stolen.txt"])
    ...
    def test_multiple_artifacts_partial_resolution(self, tmp_path):
        ...
        def resolve_side_effect(uid, tid, vpath, tenant_id=None):
        ...
        with patch("deerflow.config.paths.get_paths", return_value=mock_paths):
            result = _resolve_attachments("local", 
                thread_id,
    ...
    def test_passes_tenant_id_to_path_resolution(self, tmp_path):
        """tenant_id from caller is forwarded to path manager methods."""
        from app.channels.manager import _resolve_attachments

        thread_id = "tenant-thread"
        outputs_dir = tmp_path / "outputs"
        outputs_dir.mkdir(parents=True)
        report = outputs_dir / "report.txt"
        report.write_text("ok")

        mock_paths = MagicMock()
        mock_paths.sandbox_outputs_dir.return_value = outputs_dir
        mock_paths.resolve_virtual_path.return_value = report
        ...
        assert len(result) == 1
        mock_paths.sandbox_outputs_dir.assert_called_once_with("local", thread_id, tenant_id="tenant-a")
        mock_paths.resolve_virtual_path.assert_called_once_with(
            "local",
            thread_id,
            "/mnt/user-data/outputs/report.txt",
            tenant_id="tenant-a",
...
class TestManagerArtifactResolution:
    def test_handle_chat_populates_attachments(self):
        ...
        mock_paths = MagicMock()
        with patch("deerflow.config.paths.get_paths", return_value=mock_paths):
            result = _resolve_attachments("local", "t1", [])
        assert result == []

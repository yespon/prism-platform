...
def _make_test_app(tmp_path: Path, default_user_id: str | None = "user-a"):
...
@pytest.fixture()
def unauth_agent_client(tmp_path):
    """TestClient that does not auto-inject a fallback authenticated user."""
    paths_instance = _make_paths(tmp_path)

    with patch("deerflow.config.agents_config.get_paths", return_value=paths_instance), patch("app.gateway.routers.agents.get_paths", return_value=paths_instance):
        app = _make_test_app(tmp_path, default_user_id=None)
        with TestClient(app) as client:
            client._tmp_path = tmp_path  # type: ignore[attr-defined]
            yield client
...
class TestAgentsAPI:
    ...
    def test_create_persists_files_on_disk(self, agent_client, tmp_path):
        ...
        agent_dir = tmp_path / "tenants" / "tenant-a" / "users" / "user-a" / "agents" / "disk-check"
    ...
    def test_delete_removes_files_from_disk(self, agent_client, tmp_path):
        ...
        agent_dir = tmp_path / "tenants" / "tenant-a" / "users" / "user-a" / "agents" / "remove-me"
        assert agent_dir.exists()
...
class TestUserProfileAPI:
    ...
    def test_agents_are_isolated_between_users(self, agent_client):
        payload = {"name": "shared-name", "soul": "isolation"}
        first = agent_client.post("/api/agents", json=payload, headers={"x-user-id": "user-a"})
        assert first.status_code == 201

        second = agent_client.post("/api/agents", json=payload, headers={"x-user-id": "user-b"})
    ...
    def test_get_agent_returns_404_for_other_user(self, agent_client):
        created = agent_client.post(
            "/api/agents",
            json={"name": "private-agent", "soul": "for-user-a"},
            headers={"x-user-id": "user-a"},
        )
        assert created.status_code == 201

        read_other_user = agent_client.get(
            "/api/agents/private-agent",
            headers={"x-user-id": "user-b"},
        )
        assert read_other_user.status_code == 404

    def test_user_profile_is_not_shared_between_users(self, agent_client):
        ...
        read_b = agent_client.get("/api/user-profile", headers={"x-user-id": "user-b"})

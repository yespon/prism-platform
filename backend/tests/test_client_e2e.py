...
Only OPSINTECH_HOME is redirected to tmp_path for filesystem isolation.
...
def e2e_env(tmp_path, monkeypatch):
    ...
    - OPSINTECH_HOME → tmp_path (all thread data lands in a temp dir)
    ...
    monkeypatch.setenv("OPSINTECH_HOME", str(tmp_path))
...
class TestFileUploadIntegration:
    ...
    def test_upload_files(self, e2e_env, tmp_path):
        ...
        assert (get_paths().sandbox_uploads_dir("local", tid) / "readme.txt").exists()
    ...
    def test_upload_then_chat(self, e2e_env, tmp_path):
        ...
        c.upload_files(tid, [test_file])
        # Chat — the middleware should inject attachment context for referenced uploads
...
class TestMiddlewareChain:
    ...
    def test_thread_data_paths_in_state(self, client):
        ...
        from deerflow.config.paths import get_paths
        thread_dir = get_paths().thread_dir("local", tid)
...
class TestArtifactAccess:
    ...
    def test_get_artifact_happy_path(self, e2e_env):
        ...
        outputs_dir = get_paths().sandbox_outputs_dir("local", tid)
...
class TestConfigManagement:
    ...
    def test_update_mcp_config_writes_and_invalidates(self, e2e_env, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OPSINTECH_EXTENSIONS_CONFIG_PATH", str(config_file))

...
def test_present_files_keeps_virtual_outputs_path(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "get_paths",
        lambda: SimpleNamespace(resolve_virtual_path=lambda user_id, thread_id, path, tenant_id=None: artifact_path),
    )
    ...
    payload = json.loads(result.update["messages"][0].content)
    assert payload["type"] == "present_files_result"
    assert payload["version"] == 1
    assert payload["filepaths"] == ["/mnt/user-data/outputs/summary.json"]

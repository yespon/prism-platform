...
def test_openai_compatible_provider_clamps_excessive_max_tokens(monkeypatch):
    """OpenAI-compatible providers should clamp max_tokens to provider-safe upper bound."""
    model = ModelConfig(
        name="openai-compat-too-large",
        display_name="OpenAI Compat Too Large",
        description=None,
        use="langchain_openai:ChatOpenAI",
        model="gpt-4o-mini",
        max_tokens=256000,
    ...
    factory_module.create_chat_model(name="openai-compat-too-large")

    assert captured.get("max_tokens") == 65536


def test_openai_compatible_provider_clamps_non_positive_max_tokens(monkeypatch):
    """OpenAI-compatible providers should clamp non-positive max_tokens to 1."""
    model = ModelConfig(
        name="openai-compat-too-small",
        display_name="OpenAI Compat Too Small",
        description=None,
        use="langchain_openai:ChatOpenAI",
        model="gpt-4o-mini",
        max_tokens=0,
    ...
    factory_module.create_chat_model(name="openai-compat-too-small")

    assert captured.get("max_tokens") == 1
...
def test_chatopenai_is_wrapped_with_patched_adapter(monkeypatch):
    model = ModelConfig(
        name="openai-compat",
    ...
    cfg = _make_app_config([model])
    monkeypatch.setattr(factory_module, "get_app_config", lambda: cfg)
    monkeypatch.setattr(factory_module, "resolve_class", lambda path, base: ChatOpenAI)
    monkeypatch.setattr(factory_module, "is_tracing_enabled", lambda: False)

    instance = factory_module.create_chat_model(name="openai-compat")

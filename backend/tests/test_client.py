...
class TestMemoryManagement:
    ...
    def test_get_memory_config(self, client):
        config = MagicMock()
        config.enabled = True
        config.storage_path = ".opsintech/memory.json"
...
class TestArtifacts:
    def test_get_artifact(self, client):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            outputs = paths.sandbox_outputs_dir("local", "t1")
    ...
    def test_get_artifact_not_found(self, client):
        with tempfile.TemporaryDirectory() as tmp:
            paths = Paths(base_dir=tmp)
            paths.sandbox_user_data_dir("local", "t1").mkdir(parents=True)
...
class TestScenarioFileLifecycle:
    ...
    def test_upload_then_read_artifact(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            outputs_dir = paths.sandbox_outputs_dir("local", "t-artifact")
...
class TestScenarioThreadIsolation:
    ...
    def test_artifacts_isolated_per_thread(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            paths = Paths(base_dir=tmp)
            outputs_a = paths.sandbox_outputs_dir("local", "thread-a")
            outputs_a.mkdir(parents=True)
            paths.sandbox_user_data_dir("local", "thread-b").mkdir(parents=True)
...
class TestGatewayConformance:
    ...
    def test_get_memory_config(self, client):
        ...
        mem_cfg.storage_path = ".opsintech/memory.json"
...
class TestArtifactHardening:
    def test_artifact_directory_rejected(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            paths = Paths(base_dir=tmp)
            subdir = paths.sandbox_outputs_dir("local", "t1") / "subdir"

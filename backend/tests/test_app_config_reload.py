...
def _make_fake_platform_models(model_name: str, supports_thinking: bool):
    return [
        {
            "name": model_name,
...
def test_get_app_config_reloads_when_file_changes(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OPSINTECH_CONFIG_PATH", str(config_path))
    monkeypatch.setenv("OPSINTECH_EXTENSIONS_CONFIG_PATH", str(extensions_path))
    reset_app_config()
    ...
    try:
        ...
        monkeypatch.setattr(
            app_config_module,
            "load_platform_models_from_db",
            lambda: _make_fake_platform_models("first-model", supports_thinking=False),
        )
        ...
        monkeypatch.setattr(
            app_config_module,
            "load_platform_models_from_db",
            lambda: _make_fake_platform_models("first-model", supports_thinking=True),
        )
...
def test_get_app_config_reloads_when_config_path_changes(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OPSINTECH_EXTENSIONS_CONFIG_PATH", str(extensions_path))
    monkeypatch.setenv("OPSINTECH_CONFIG_PATH", str(config_a))
    reset_app_config()
    ...
    try:
        ...
        monkeypatch.setattr(
            app_config_module,
            "load_platform_models_from_db",
            lambda: _make_fake_platform_models("model-a", supports_thinking=False),
        )

        first = get_app_config()
        assert first.models[0].name == "model-a"

        monkeypatch.setenv("OPSINTECH_CONFIG_PATH", str(config_b))

        monkeypatch.setattr(
            app_config_module,
            "load_platform_models_from_db",
            lambda: _make_fake_platform_models("model-b", supports_thinking=True),
        )

...
class ExtensionsConfig(BaseModel):
    ...
    def resolve_config_path(cls, config_path: str | None = None) -> Path | None:
        ...
        2. If provided `OPSINTECH_EXTENSIONS_CONFIG_PATH` environment variable, use it.
        ...
        if config_path:
            ...
            return path
        elif os.getenv("OPSINTECH_EXTENSIONS_CONFIG_PATH"):
            path = Path(os.getenv("OPSINTECH_EXTENSIONS_CONFIG_PATH"))
            if not path.exists():
                raise FileNotFoundError(f"Extensions config file specified by environment variable `OPSINTECH_EXTENSIONS_CONFIG_PATH` not found at {path}")
            return path
        else:
...
_user_extensions_config_cache: dict[str, ExtensionsConfig] = {}


def _get_file_backed_extensions_config() -> ExtensionsConfig:
...
def _get_user_scoped_extensions_config(user_id: str) -> ExtensionsConfig:
    global _user_extensions_config_cache

    tenant_id = get_current_tenant_id()
    cache_key = f"{tenant_id}:{user_id}"

    cached = _user_extensions_config_cache.get(cache_key)
    if cached is not None:
        return cached

    payload = load_user_config_payload(user_id, tenant_id=tenant_id)
    if payload is None:
        return _get_file_backed_extensions_config()

    _, ext_payload = payload
    if not isinstance(ext_payload, dict):
        return _get_file_backed_extensions_config()

    user_extensions = ExtensionsConfig.model_validate(ext_payload)
    _user_extensions_config_cache[cache_key] = user_extensions
...
def get_extensions_config() -> ExtensionsConfig:
    ...
    if user_id:
        return _get_user_scoped_extensions_config(user_id)
...
def reload_extensions_config(config_path: str | None = None) -> ExtensionsConfig:
    ...
    """
    global _extensions_config, _user_extensions_config_cache
    _extensions_config = ExtensionsConfig.from_file(config_path)
    _user_extensions_config_cache = {}
    return _extensions_config

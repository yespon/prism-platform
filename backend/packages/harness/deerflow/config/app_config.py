...
from deerflow.database.user_config_store import load_user_config_payload, load_platform_models_from_db

load_dotenv()

logger = logging.getLogger(__name__)

_secrets_key_warning_emitted = False
...
class AppConfig(BaseModel):
    ...
    def resolve_config_path(cls, config_path: str | None = None) -> Path:
        ...
        2. If provided `OPSINTECH_CONFIG_PATH` environment variable, use it.
        ...
        if config_path:
            ...
            return path
        elif os.getenv("OPSINTECH_CONFIG_PATH"):
            path = Path(os.getenv("OPSINTECH_CONFIG_PATH"))
            if not Path.exists(path):
                raise FileNotFoundError(f"Config file specified by environment variable `OPSINTECH_CONFIG_PATH` not found at {path}")
            return path
        else:
    ...
    def from_file(cls, config_path: str | None = None) -> Self:
        ...
        config_data.pop("extensions", None)
...
_user_app_config_cache: dict[str, AppConfig] = {}
...
def _load_and_cache_app_config(config_path: str | None = None) -> AppConfig:
    ...
    global _app_config, _app_config_path, _app_config_mtime, _app_config_is_custom, _user_app_config_cache
    ...
    _app_config_is_custom = False
    _user_app_config_cache = {}
    _warn_if_missing_secrets_key_for_prod()
    return _app_config
...
def _warn_if_missing_secrets_key_for_prod() -> None:
    global _secrets_key_warning_emitted
    if _secrets_key_warning_emitted:
        return
    ...
    logger.warning(
        "Production environment detected but DEERFLOW_SECRETS_ENCRYPTION_KEY is not configured. "
        "Secret fields may be stored without encryption-at-rest."
    )
    _secrets_key_warning_emitted = True


def _get_base_app_config() -> AppConfig:
    """Get base app config from file-backed singleton cache."""
    ...
    platform_models = load_platform_models_from_db()
    if platform_models:
        _app_config.models = [ModelConfig(**m) for m in platform_models]
    
    return _app_config


def _get_user_scoped_app_config(user_id: str) -> AppConfig:
    """Overlay DB user payload onto base config while preserving AppConfig schema."""
    global _user_app_config_cache

    tenant_id = get_current_tenant_id()
    cache_key = f"{tenant_id}:{user_id}"

    cached = _user_app_config_cache.get(cache_key)
    if cached is not None:
        base_config = _get_base_app_config()
        base_model_names = {m.name for m in base_config.models}
        cached_model_names = {m.name for m in cached.models}
        if base_model_names != cached_model_names:
            logger.info(
                "Platform models changed for user %s (cached: %s, current: %s), invalidating cache",
                user_id,
                cached_model_names,
                base_model_names,
            )
            _user_app_config_cache.pop(cache_key, None)
        else:
            return cached

    base_config = _get_base_app_config()
    payload = load_user_config_payload(user_id, tenant_id=tenant_id)
    if payload is None:
        return base_config
    ...
    if isinstance(app_payload, dict):
        user_models = app_payload.pop("models", None)
        if user_models is not None and user_models:
            platform_models = merged.get("models", [])
            seen_names = set()
            combined = []
            ...
            for m in user_models:
                ...
                if name and name not in seen_names:
                    seen_names.add(name)
                    combined.append(m)
            merged["models"] = combined
        merged.update(app_payload)
...
def get_app_config() -> AppConfig:
    ...
    if user_id:
        return _get_user_scoped_app_config(user_id)

    if _app_config is not None and _app_config_is_custom:
        return _app_config

    return _get_base_app_config()

...
def _filter_messages_for_memory(messages: list[Any]) -> list[Any]:
    ...
    - AI messages with tool_calls (intermediate steps, not final responses)
        - Legacy <uploaded_files> blocks in historical human messages
    ...
    """
    _LEGACY_UPLOAD_BLOCK_RE = re.compile(r"<uploaded_files>[\s\S]*?</uploaded_files>\n*", re.IGNORECASE)
    ...
    for msg in messages:
        ...
        if msg_type == "human":
            ...
            if "<uploaded_files>" in content_str:
                # Backward compatibility: strip legacy upload block from old turns.
                stripped = _LEGACY_UPLOAD_BLOCK_RE.sub("", content_str).strip()
                if not stripped:

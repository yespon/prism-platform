...
class OAuthTokenManager:
    ...
    @classmethod
    def from_dict(cls, mcp_servers: dict[str, dict[str, Any]]) -> OAuthTokenManager:
        """Create OAuthTokenManager from dictionary configuration.
        ...
        for server_name, server_config in mcp_servers.items():
            ...
            if oauth_config and oauth_config.get("enabled", True):
                # Convert dict to McpOAuthConfig
                oauth_by_server[server_name] = McpOAuthConfig(**oauth_config)
        return cls(oauth_by_server)
...
def build_oauth_tool_interceptor_from_dict(mcp_servers: dict[str, dict[str, Any]]) -> Any | None:
    """Build a tool interceptor that injects OAuth Authorization headers from dictionary config."""
    token_manager = OAuthTokenManager.from_dict(mcp_servers)
    if not token_manager.has_oauth_servers():
        return None
...
async def get_initial_oauth_headers_from_dict(mcp_servers: dict[str, dict[str, Any]]) -> dict[str, str]:
    """Get initial OAuth Authorization headers for MCP server connections from dictionary config."""
    token_manager = OAuthTokenManager.from_dict(mcp_servers)
    if not token_manager.has_oauth_servers():
        return {}

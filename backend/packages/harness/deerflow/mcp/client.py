...
def build_server_params_from_dict(server_name: str, config: dict[str, Any]) -> dict[str, Any]:
    """Build server parameters for MultiServerMCPClient from dictionary.

    Args:
        server_name: Name of the MCP server.
        config: Dictionary configuration for the MCP server.

    Returns:
        Dictionary of server parameters for langchain-mcp-adapters.
    """
    transport_type = config.get("type") or config.get("transport") or "stdio"
    params: dict[str, Any] = {"transport": transport_type}
...
def build_servers_config_from_dict(mcp_servers: dict[str, dict[str, Any]]) -> dict[str, dict[str, Any]]:
    """Build servers configuration for MultiServerMCPClient from dictionary.
    ...
    for server_name, server_config in mcp_servers.items():
        ...
        if not server_config.get("enabled", True):
            continue
        try:
            servers_config[server_name] = build_server_params_from_dict(server_name, server_config)
            logger.info(f"Configured MCP server: {server_name}")
        except Exception as e:
            logger.error(f"Failed to configure MCP server '{server_name}': {e}")

...
DEFAULT_PORT = 8080
DEFAULT_CONTAINER_PREFIX = "opsintech-sandbox"
...
class AioSandboxProvider(SandboxProvider):
    ...
    def __init__(self):
        ...
        self._thread_sandboxes: dict[tuple[str, str, str], str] = {}  # (tenant_id, user_id, thread_id) -> sandbox_id
        self._thread_locks: dict[tuple[str, str, str], threading.Lock] = {}  # (tenant_id, user_id, thread_id) -> lock
    ...
    @staticmethod
    def _deterministic_sandbox_id(thread_id: str, user_id: str, tenant_id: str | None = None) -> str:
        """Generate a deterministic sandbox ID from user_id + thread_id.

        Ensures all processes derive the same sandbox_id for a given user/thread,
    ...
    def _get_extra_mounts(
        ...
        if thread_id and user_id:
            mounts.extend(self._get_thread_mounts(thread_id, user_id, tenant_id))
            logger.info(f"Adding thread mounts for user={user_id}, thread={thread_id}: {mounts}")
    ...
    def _get_thread_mounts(
        ...
        """
        paths = get_paths()
        paths.ensure_thread_dirs(user_id, thread_id, tenant_id=tenant_id)

        # host_paths resolves to the host-side base dir when OPSINTECH_HOST_BASE_DIR
        # is set, otherwise falls back to the container's own base dir (native mode).
        host_paths = Paths(base_dir=paths.host_base_dir)

        return [
            (
                str(host_paths.sandbox_work_dir(user_id, thread_id, tenant_id=tenant_id)),
                f"{VIRTUAL_PATH_PREFIX}/workspace",
                False,
            ),
            (
                str(host_paths.sandbox_uploads_dir(user_id, thread_id, tenant_id=tenant_id)),
                f"{VIRTUAL_PATH_PREFIX}/uploads",
                False,
            ),
            (
                str(host_paths.sandbox_outputs_dir(user_id, thread_id, tenant_id=tenant_id)),
    ...
    def _get_skills_mount() -> tuple[str, str, bool] | None:
        ...
        Mount source uses OPSINTECH_HOST_SKILLS_PATH when running inside Docker (DooD)
        ...
        try:
            ...
            if skills_path.exists():
                ...
                host_skills = os.environ.get("OPSINTECH_HOST_SKILLS_PATH") or str(skills_path)
    ...
    def _thread_key(self, user_id: str, thread_id: str, tenant_id: str | None) -> tuple[str, str, str]:
        return (tenant_id or "default", user_id, thread_id)

    def _get_thread_lock(self, user_id: str, thread_id: str, tenant_id: str | None) -> threading.Lock:
        """Get or create an in-process lock for a specific user/thread pair."""
        thread_key = self._thread_key(user_id, thread_id, tenant_id)
        with self._lock:
            if thread_key not in self._thread_locks:
                self._thread_locks[thread_key] = threading.Lock()
    ...
    def acquire(
        ...
        if thread_id:
            ...
            thread_lock = self._get_thread_lock(user_id, thread_id, tenant_id)
            with thread_lock:
                return self._acquire_internal(thread_id, user_id, tenant_id)
        else:
            return self._acquire_internal(thread_id, user_id, tenant_id)
    ...
    def _acquire_internal(
        ...
        if thread_id:
            thread_key = self._thread_key(user_id, thread_id, tenant_id)
            with self._lock:
                if thread_key in self._thread_sandboxes:
                    existing_id = self._thread_sandboxes[thread_key]
                    if existing_id in self._sandboxes:
                        logger.info(f"Reusing in-process sandbox {existing_id} for user={user_id}, thread={thread_id}")
                        self._last_activity[existing_id] = time.time()
                        return existing_id
                    else:
                        del self._thread_sandboxes[thread_key]

        # Deterministic ID for thread-specific, random for anonymous
        sandbox_id = (
            self._deterministic_sandbox_id(thread_id, user_id, tenant_id)
            if thread_id and user_id
        ...
        if thread_id:
            with self._lock:
                if sandbox_id in self._warm_pool:
                    ...
                    self._thread_sandboxes[self._thread_key(user_id, thread_id, tenant_id)] = sandbox_id
                    logger.info(f"Reclaimed warm-pool sandbox {sandbox_id} for user={user_id}, thread={thread_id} at {info.sandbox_url}")
                    return sandbox_id
        ...
        if thread_id:
            return self._discover_or_create_with_lock(thread_id, user_id, tenant_id, sandbox_id)

        return self._create_sandbox(thread_id, user_id, tenant_id, sandbox_id)
    ...
    def _discover_or_create_with_lock(
        ...
        paths = get_paths()
        paths.ensure_thread_dirs(user_id, thread_id, tenant_id=tenant_id)
        lock_path = paths.thread_dir(user_id, thread_id, tenant_id=tenant_id) / f"{sandbox_id}.lock"
        thread_key = self._thread_key(user_id, thread_id, tenant_id)
            ...
            try:
                ...
                with self._lock:
                    if thread_key in self._thread_sandboxes:
                        existing_id = self._thread_sandboxes[thread_key]
                        if existing_id in self._sandboxes:
                            logger.info(f"Reusing in-process sandbox {existing_id} for user={user_id}, thread={thread_id} (post-lock check)")
                            self._last_activity[existing_id] = time.time()
                            return existing_id
                    if sandbox_id in self._warm_pool:
                        info, _ = self._warm_pool.pop(sandbox_id)
                        ...
                        self._sandbox_infos[sandbox_id] = info
                        self._last_activity[sandbox_id] = time.time()
                        self._thread_sandboxes[thread_key] = sandbox_id
                        logger.info(f"Reclaimed warm-pool sandbox {sandbox_id} for user={user_id}, thread={thread_id} (post-lock check)")
                        return sandbox_id
                ...
                if discovered is not None:
                    ...
                    with self._lock:
                        ...
                        self._thread_sandboxes[thread_key] = discovered.sandbox_id
                    logger.info(f"Discovered existing sandbox {discovered.sandbox_id} for user={user_id}, thread={thread_id} at {discovered.sandbox_url}")
    ...
    def _create_sandbox(
        ...
        """
        extra_mounts = self._get_extra_mounts(thread_id, user_id, tenant_id)
        ...
        logger.info(f"Created sandbox {sandbox_id} for user={user_id}, thread={thread_id} at {info.sandbox_url}")
        return sandbox_id

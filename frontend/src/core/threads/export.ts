...
function extractMessageAttachments(message: Message): Array<Record<string, unknown>> {
    ...
    if (message.type === "human") {
      ...
      if (content) {
        ...
        if (attachments.length > 0) {
          lines.splice(lines.length - 2, 0, "### Attachments", "");
          for (const attachment of attachments) {
            const name =
              typeof attachment.filename === "string"
                ...
                : "unknown";
            const path =
              typeof attachment.virtual_path === "string"
            ...
            lines.splice(
              lines.length - 2,
              0,
              `- ${name}${path ? ` (${path})` : ""}${attachmentId ? ` [${attachmentId}]` : ""}`,
            );
          }
